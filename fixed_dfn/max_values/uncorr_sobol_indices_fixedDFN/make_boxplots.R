...
g2 = ggplot(graph_data, aes(x = input_var, y = value, fill = output_var)) + geom_boxplot(lwd=0.25, outlier.size = 0.2, width = 0.9) + 
  ggtitle(TeX("")) + 
  theme_bw() + 
  xlab("") + 
  ylab("") + 
  ...
  labs(fill="Flush Time")+ guides(fill="none")  + scale_fill_manual(name = "max_calcite",values = c("grey"))
...
g4 = ggplot(bootstrapped_TEmean, aes(x = factor(input_var, levels = c('Da_1_gypsum', 'Da_1_calcite',
                                                                      ...
                                                                      'tau',
                                                                      'epsilon')), 
                                     y = value, fill = output_var)) + geom_boxplot(lwd=0.25, outlier.size = 0.2, width = 0.9) + 
  ggtitle(TeX("")) + 
  theme_bw() + 
...
g4 = g4 + scale_x_discrete(labels = TeX(c('Pe' = r"(Pe)", 
                                          ...
                                          'tau' = r"($\tau$)",
                                          'epsilon' = r"($\epsilon$)")) )
# grid.arrange(g2, g4, nrow = 1, top = textGrob("FIXED DFN: Sobol' Indices for Calcite Precipitation at Different Flush Times for Non-Functional Variables", gp=gpar(fontsize=15,font=8)))
