...
library(tidyverse)
library(latex2exp)
setwd("~/GitLab/sa_for_chemistry/variable_dfn_regulartime/eda")

# Data location:
data_path = "~/GitLab/sa_for_chemistry/variable_dfn_regulartime/data"
...
outputs_abrv    = c("flush_0", "flush_1",
                    ...
                    "flush_10", "flush_11",   
                    "flush_12")
calcite_outputs = c("calcite_flush_0",      "calcite_flush_1", 
                    "calcite_flush_2",    
                    "calcite_flush_3",     "calcite_flush_4",
                    "calcite_flush_5",     "calcite_flush_6")
...
calcite_outputs_modified = c("calcite_flush_0", "calcite_flush_1",           
                             "calcite_flush_2",     
                             "calcite_flush_3",
                             "calcite_flush_4", "calcite_flush_5",            
                             "calcite_flush_6")
...
# yy                      = melt(gypsum_data, id = "sim_num")
# yy$variable             = factor(yy$variable)
# yy$value[which(yy$value == 0)] = NA
# levels(yy$variable) = outputs_abrv
# 
# var_gypsums = ggplot(yy, aes(x = factor(variable), y = value, color = sim_num, group = sim_num)) + 
#   geom_line()+xlab(TeX("$\\tau$"))+ 
#   theme_bw()+
#   theme(legend.position = 'none',
#         axis.text.x = element_text(angle = 270, vjust = 0.5, hjust=1),
#         axis.title=element_text(size=14))+ 
#   scale_color_grey() + 
#   ggtitle("Gypsum Curves for Variable Fracture Networks")
...
yy$variable             = factor(yy$variable)
levels(yy$variable) = outputs_abrv
g2 = ggplot(yy, aes(x = factor(variable), y = value, color = sim_num, group = sim_num))+ 
  scale_color_grey() + 
  geom_line()+xlab(TeX("Time [y]"))+ylab(TeX("Calcite Minearlized"))+ 
  theme_bw()+
  theme(legend.position = 'none',
  ...
  ggtitle("") 
g2 = g2 + scale_x_discrete(labels = TeX(c('flush_0' = r"($10^{-7}$)",
                                          'flush_1' = r"($10^{-6}$)",
                                          'flush_2' = r"($10^{-5}$)",
                                          'flush_3' = r"($10^{-4}$)",
                                          'flush_4' = r"($10^{-3}$)",
